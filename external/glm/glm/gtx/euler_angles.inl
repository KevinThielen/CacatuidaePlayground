...
#include "compatibility.hpp" // cac::atan2
...
namespace cac
{
	...
	{
		T cosX = cac::cos(angleX);
		T sinX = cac::sin(angleX);
	...
	{
		T cosY = cac::cos(angleY);
		T sinY = cac::sin(angleY);
	...
	{
		T cosZ = cac::cos(angleZ);
		T sinZ = cac::sin(angleZ);
	...
	{
		T cosX = cac::cos(angleX) * angularVelocityX;
		T sinX = cac::sin(angleX) * angularVelocityX;
	...
	{
		T cosY = cac::cos(angleY) * angularVelocityY;
		T sinY = cac::sin(angleY) * angularVelocityY;
	...
	{
		T cosZ = cac::cos(angleZ) * angularVelocityZ;
		T sinZ = cac::sin(angleZ) * angularVelocityZ;
	...
	{
		T tmp_ch = cac::cos(yaw);
		T tmp_sh = cac::sin(yaw);
		T tmp_cp = cac::cos(pitch);
		T tmp_sp = cac::sin(pitch);
		T tmp_cb = cac::cos(roll);
		T tmp_sb = cac::sin(roll);
    ...
    GLM_FUNC_DECL void extractEulerAngleXYZ(mat<4, 4, T, defaultp> const& M,
                                            ...
                                            T & t3)
    {
        T T1 = cac::atan2<T, defaultp>(M[2][1], M[2][2]);
        T C2 = cac::sqrt(M[0][0]*M[0][0] + M[1][0]*M[1][0]);
        T T2 = cac::atan2<T, defaultp>(-M[2][0], C2);
        T S1 = cac::sin(T1);
        T C1 = cac::cos(T1);
        T T3 = cac::atan2<T, defaultp>(S1*M[0][2] - C1*M[0][1], C1*M[1][1] - S1*M[1][2  ]);
        t1 = -T1;
        t2 = -T2;
        t3 = -T3;
    }
	...
	GLM_FUNC_QUALIFIER void extractEulerAngleYXZ(mat<4, 4, T, defaultp> const & M,
												 ...
												 T & t3)
	{
		T T1 = cac::atan2<T, defaultp>(M[2][0], M[2][2]);
		T C2 = cac::sqrt(M[0][1]*M[0][1] + M[1][1]*M[1][1]);
		T T2 = cac::atan2<T, defaultp>(-M[2][1], C2);
		T S1 = cac::sin(T1);
		T C1 = cac::cos(T1);
		T T3 = cac::atan2<T, defaultp>(S1*M[1][2] - C1*M[1][0], C1*M[0][0] - S1*M[0][2]);
		t1 = T1;
		t2 = T2;
		t3 = T3;
	}
	...
	GLM_FUNC_QUALIFIER void extractEulerAngleXZX(mat<4, 4, T, defaultp> const & M,
												 ...
												 T & t3)
	{
		T T1 = cac::atan2<T, defaultp>(M[0][2], M[0][1]);
		T S2 = cac::sqrt(M[1][0]*M[1][0] + M[2][0]*M[2][0]);
		T T2 = cac::atan2<T, defaultp>(S2, M[0][0]);
		T S1 = cac::sin(T1);
		T C1 = cac::cos(T1);
		T T3 = cac::atan2<T, defaultp>(C1*M[1][2] - S1*M[1][1], C1*M[2][2] - S1*M[2][1]);
		t1 = T1;
		t2 = T2;
		t3 = T3;
	}
	...
	GLM_FUNC_QUALIFIER void extractEulerAngleXYX(mat<4, 4, T, defaultp> const & M,
												 ...
												 T & t3)
	{
		T T1 = cac::atan2<T, defaultp>(M[0][1], -M[0][2]);
		T S2 = cac::sqrt(M[1][0]*M[1][0] + M[2][0]*M[2][0]);
		T T2 = cac::atan2<T, defaultp>(S2, M[0][0]);
		T S1 = cac::sin(T1);
		T C1 = cac::cos(T1);
		T T3 = cac::atan2<T, defaultp>(-C1*M[2][1] - S1*M[2][2], C1*M[1][1] + S1*M[1][2]);
		t1 = T1;
		t2 = T2;
		t3 = T3;
	}
	...
	GLM_FUNC_QUALIFIER void extractEulerAngleYXY(mat<4, 4, T, defaultp> const & M,
												 ...
												 T & t3)
	{
		T T1 = cac::atan2<T, defaultp>(M[1][0], M[1][2]);
		T S2 = cac::sqrt(M[0][1]*M[0][1] + M[2][1]*M[2][1]);
		T T2 = cac::atan2<T, defaultp>(S2, M[1][1]);
		T S1 = cac::sin(T1);
		T C1 = cac::cos(T1);
		T T3 = cac::atan2<T, defaultp>(C1*M[2][0] - S1*M[2][2], C1*M[0][0] - S1*M[0][2]);
		t1 = T1;
		t2 = T2;
		t3 = T3;
	}
	...
	GLM_FUNC_QUALIFIER void extractEulerAngleYZY(mat<4, 4, T, defaultp> const & M,
												 ...
												 T & t3)
	{
		T T1 = cac::atan2<T, defaultp>(M[1][2], -M[1][0]);
		T S2 = cac::sqrt(M[0][1]*M[0][1] + M[2][1]*M[2][1]);
		T T2 = cac::atan2<T, defaultp>(S2, M[1][1]);
		T S1 = cac::sin(T1);
		T C1 = cac::cos(T1);
		T T3 = cac::atan2<T, defaultp>(-S1*M[0][0] - C1*M[0][2], S1*M[2][0] + C1*M[2][2]);
		t1 = T1;
		t2 = T2;
		t3 = T3;
	}
	...
	GLM_FUNC_QUALIFIER void extractEulerAngleZYZ(mat<4, 4, T, defaultp> const & M,
												 ...
												 T & t3)
	{
		T T1 = cac::atan2<T, defaultp>(M[2][1], M[2][0]);
		T S2 = cac::sqrt(M[0][2]*M[0][2] + M[1][2]*M[1][2]);
		T T2 = cac::atan2<T, defaultp>(S2, M[2][2]);
		T S1 = cac::sin(T1);
		T C1 = cac::cos(T1);
		T T3 = cac::atan2<T, defaultp>(C1*M[0][1] - S1*M[0][0], C1*M[1][1] - S1*M[1][0]);
		t1 = T1;
		t2 = T2;
		t3 = T3;
	}
	...
	GLM_FUNC_QUALIFIER void extractEulerAngleZXZ(mat<4, 4, T, defaultp> const & M,
												 ...
												 T & t3)
	{
		T T1 = cac::atan2<T, defaultp>(M[2][0], -M[2][1]);
		T S2 = cac::sqrt(M[0][2]*M[0][2] + M[1][2]*M[1][2]);
		T T2 = cac::atan2<T, defaultp>(S2, M[2][2]);
		T S1 = cac::sin(T1);
		T C1 = cac::cos(T1);
		T T3 = cac::atan2<T, defaultp>(-C1*M[1][0] - S1*M[1][1], C1*M[0][0] + S1*M[0][1]);
		t1 = T1;
		t2 = T2;
		t3 = T3;
	}
	...
	GLM_FUNC_QUALIFIER void extractEulerAngleXZY(mat<4, 4, T, defaultp> const & M,
												 ...
												 T & t3)
	{
		T T1 = cac::atan2<T, defaultp>(M[1][2], M[1][1]);
		T C2 = cac::sqrt(M[0][0]*M[0][0] + M[2][0]*M[2][0]);
		T T2 = cac::atan2<T, defaultp>(-M[1][0], C2);
		T S1 = cac::sin(T1);
		T C1 = cac::cos(T1);
		T T3 = cac::atan2<T, defaultp>(S1*M[0][1] - C1*M[0][2], C1*M[2][2] - S1*M[2][1]);
		t1 = T1;
		t2 = T2;
		t3 = T3;
	}
	...
	GLM_FUNC_QUALIFIER void extractEulerAngleYZX(mat<4, 4, T, defaultp> const & M,
												 ...
												 T & t3)
	{
		T T1 = cac::atan2<T, defaultp>(-M[0][2], M[0][0]);
		T C2 = cac::sqrt(M[1][1]*M[1][1] + M[2][1]*M[2][1]);
		T T2 = cac::atan2<T, defaultp>(M[0][1], C2);
		T S1 = cac::sin(T1);
		T C1 = cac::cos(T1);
		T T3 = cac::atan2<T, defaultp>(S1*M[1][0] + C1*M[1][2], S1*M[2][0] + C1*M[2][2]);
		t1 = T1;
		t2 = T2;
		t3 = T3;
	}
	...
	GLM_FUNC_QUALIFIER void extractEulerAngleZYX(mat<4, 4, T, defaultp> const & M,
												 ...
												 T & t3)
	{
		T T1 = cac::atan2<T, defaultp>(M[0][1], M[0][0]);
		T C2 = cac::sqrt(M[1][2]*M[1][2] + M[2][2]*M[2][2]);
		T T2 = cac::atan2<T, defaultp>(-M[0][2], C2);
		T S1 = cac::sin(T1);
		T C1 = cac::cos(T1);
		T T3 = cac::atan2<T, defaultp>(S1*M[2][0] - C1*M[2][1], C1*M[1][1] - S1*M[1][0]);
		t1 = T1;
		t2 = T2;
		t3 = T3;
	}
	...
	GLM_FUNC_QUALIFIER void extractEulerAngleZXY(mat<4, 4, T, defaultp> const & M,
												 ...
												 T & t3)
	{
		T T1 = cac::atan2<T, defaultp>(-M[1][0], M[1][1]);
		T C2 = cac::sqrt(M[0][2]*M[0][2] + M[2][2]*M[2][2]);
		T T2 = cac::atan2<T, defaultp>(M[1][2], C2);
		T S1 = cac::sin(T1);
		T C1 = cac::cos(T1);
		T T3 = cac::atan2<T, defaultp>(C1*M[2][0] + S1*M[2][1], C1*M[0][0] + S1*M[0][1]);
		t1 = T1;
		t2 = T2;
		t3 = T3;
	}
