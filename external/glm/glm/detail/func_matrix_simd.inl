...
#	if GLM_CONFIG_ALIGNED_GENTYPES == GLM_ENABLE
	...
	struct compute_matrixCompMult<4, 4, float, Q, true>
	{
		...
		GLM_FUNC_QUALIFIER static mat<4, 4, float, Q> call(mat<4, 4, float, Q> const& x, mat<4, 4, float, Q> const& y)
		{
			...
			cac_mat4_matrixCompMult(
				*static_cast<cac_vec4 const (*)[4]>(&x[0].data),
				*static_cast<cac_vec4 const (*)[4]>(&y[0].data),
				*static_cast<cac_vec4(*)[4]>(&Result[0].data));
			return Result;
		}
	};
#	endif
	...
	struct compute_transpose<4, 4, float, Q, true>
	{
		...
		{
			mat<4, 4, float, Q> Result;
			cac_mat4_transpose(&m[0].data, &Result[0].data);
			return Result;
		}
	};
	...
	struct compute_determinant<4, 4, float, Q, true>
	{
		...
		{
			return _mm_cvtss_f32(cac_mat4_determinant(&m[0].data));
		}
	};
	...
	struct compute_inverse<4, 4, float, Q, true>
	{
		...
		{
			mat<4, 4, float, Q> Result;
			cac_mat4_inverse(&m[0].data, &Result[0].data);
			return Result;
		}
	};
}//namespace detail
...
#	if GLM_CONFIG_ALIGNED_GENTYPES == GLM_ENABLE
	...
	GLM_FUNC_QUALIFIER mat<4, 4, float, aligned_lowp> outerProduct<4, 4, float, aligned_lowp>(vec<4, float, aligned_lowp> const& c, vec<4, float, aligned_lowp> const& r)
	{
		...
		cac_mat4_outerProduct(c.data, r.data, NativeResult);
