/// @ref core
/// @file cac/detail/func_geometric_simd.inl
...
namespace detail
{
	...
	struct compute_length<4, float, Q, true>
	{
		...
		{
			return _mm_cvtss_f32(cac_vec4_length(v.data));
		}
	};
	...
	struct compute_distance<4, float, Q, true>
	{
		...
		{
			return _mm_cvtss_f32(cac_vec4_distance(p0.data, p1.data));
		}
	};
	...
	struct compute_cross<float, Q, true>
	{
		GLM_FUNC_QUALIFIER static vec<3, float, Q> call(vec<3, float, Q> const& a, vec<3, float, Q> const& b)
		{
			...
			__m128 const xpd0 = cac_vec4_cross(set0, set1);
	...
	struct compute_normalize<4, float, Q, true>
	{
		...
		{
			vec<4, float, Q> Result;
			Result.data = cac_vec4_normalize(v.data);
			return Result;
		}
	};
	...
	struct compute_faceforward<4, float, Q, true>
	{
		...
		{
			vec<4, float, Q> Result;
			Result.data = cac_vec4_faceforward(N.data, I.data, Nref.data);
			return Result;
		}
	};
	...
	struct compute_reflect<4, float, Q, true>
	{
		...
		{
			vec<4, float, Q> Result;
			Result.data = cac_vec4_reflect(I.data, N.data);
			return Result;
		}
	};
	...
	struct compute_refract<4, float, Q, true>
	{
		...
		{
			vec<4, float, Q> Result;
			Result.data = cac_vec4_refract(I.data, N.data, _mm_set1_ps(eta));
			return Result;
		}
	};
}//namespace detail
