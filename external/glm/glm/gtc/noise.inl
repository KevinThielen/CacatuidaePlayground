...
namespace gtc
{
	...
	{
		vec<4, T, Q> Pi = cac::floor(vec<4, T, Q>(Position.x, Position.y, Position.x, Position.y)) + vec<4, T, Q>(0.0, 0.0, 1.0, 1.0);
		vec<4, T, Q> Pf = cac::fract(vec<4, T, Q>(Position.x, Position.y, Position.x, Position.y)) - vec<4, T, Q>(0.0, 0.0, 1.0, 1.0);
		...
		vec<4, T, Q> gx = static_cast<T>(2) * cac::fract(i / T(41)) - T(1);
		vec<4, T, Q> gy = cac::abs(gx) - T(0.5);
		vec<4, T, Q> tx = cac::floor(gx + T(0.5));
		gx = gx - tx;
	...
	*/
	...
	template<typename T, qualifier Q>
	GLM_FUNC_QUALIFIER T simplex(cac::vec<2, T, Q> const& v)
	{
